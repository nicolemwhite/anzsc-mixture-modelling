#1_motviation_example.R
library(tidyverse)

m1 = 27
s1 = 2
m2 = 34
s2 = 4

n = 10000
eta = 0.3

z = sample(1:2,n,eta)

y.mix = (z==1)*rnorm(n,m1,s1) + (z==2)*rnorm(n,m2,s2)

dat = tibble(z=factor(z),y=y.mix)


#overall distribution, no classification
ggplot(dat, aes(y)) + 
  geom_histogram(binwidth=1,alpha = 0.3,colour='black') + theme_minimal()+
  scale_x_continuous('Body Mass Index (BMI)',breaks=seq(15,50,5))+
  scale_y_continuous('Frequency',breaks=seq(0,1200,200))+theme(axis.text = element_text(size=12))

ggsave("figures/motivation.null.png", width = 20, height = 20, units = "cm")
invisible(dev.off())


#known classification
ggplot(dat, aes(y)) + 
  geom_histogram(data = filter(dat,z=='1'), fill = "red", alpha = 0.3,binwidth=1) + 
  geom_histogram(data = filter(dat,z=='2'), fill = "blue", alpha = 0.3,binwidth=1) +theme_minimal()+
  scale_x_continuous('Body Mass Index (BMI)',breaks=seq(15,50,5))+
  scale_y_continuous('Frequency',breaks=seq(0,1200,200))+theme(axis.text = element_text(size=12))
ggsave("figures/motivation.z.png", width = 20, height = 20, units = "cm")
invisible(dev.off())